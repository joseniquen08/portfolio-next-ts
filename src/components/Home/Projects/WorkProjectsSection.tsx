...
export const WorkProjectsSection = ({ locale }: Props) => {
  const [workProjectsSort, setWorkProjectsSort] = useState<ProjectType[]>(workProjects);
  ...
  const onValueChange = (value: string) => {
    if (value !== position) {
      if (value === "toRecent") {
        setPosition(value);
        setWorkProjectsSort(workProjectsSort.reverse());
      } else if (value === "toOld") {
        setPosition(value);
        setWorkProjectsSort(workProjectsSort.reverse());
      }
    ...
    <div className="flex flex-col gap-2 md:gap-4 border border-gray-200 dark:border-white/10 p-3.5 rounded-xl">
      ...
      <ScrollArea className="h-152 md:h-136">
        <div className="px-1.5">
          <Timeline>
            {workProjectsSort.map(({ title, badges, slug, description, github, web, icons, images }, i) => (
              <ProjectRow
                key={i}
                badges={badges}
                slug={slug}
